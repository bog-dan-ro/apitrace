...
bool AndroidUtils::updateFile(const QString &localPath, const QString &devicePath, UpdateType updateType)
{
    ...
    case PushLocalToDevice:
        ...
        emit statusMessage(tr("Please wait ... Pushing file: \"%1\" to :\"%2\"").arg(localPath).arg(devicePath));
        if (!runAdb(QStringList()<< _("push") << localPath << devicePath))
            return false;
    ...
    case PullFromDeviceToLocal:
        ...
        emit statusMessage(tr("Please wait ... Pulling file: \"%1\" to :\"%2\"").arg(devicePath).arg(localPath));
        if (!runAdb(QStringList()<< _("pull") << devicePath << localPath))
            return false;
